...
#             # Chờ 1 giây sau khi đã đứng ổn định mới chụp
...
#                 # Lấy 3 frame để trung bình cho đỡ mờ
#                 face_frames = []
#                 for _ in range(3):
#                     ok2, frame2 = cap.read()
#                     if ok2:
...
#                 if face_frames:
#                     # Làm mượt ảnh trung bình để rõ hơn
...
while True:
    ...
    for pb in person_boxes:
        ...
        # if len(tracks[key]['heights']) == 5 and not standing:
        #     tracks[key]['baseline'] = np.median(tracks[key]['heights'])
        # CHỈ cập nhật baseline khi đang ngồi ổn định trong ít nhất 1 giây
        # --- CẬP NHẬT BASELINE AN TOÀN ---
        if not standing:

            # Nếu vừa chuyển từ đứng sang ngồi → reset thời gian
            if tracks[key].get('sit_start') is None:
                tracks[key]['sit_start'] = time.time()
            
            # Tính thời gian ngồi
            sit_time = time.time() - tracks[key]['sit_start']

            # Chỉ cập nhật baseline khi ngồi ổn định >= 1 giây
            if sit_time >= 1.0 and len(tracks[key]['heights']) == 5:
                tracks[key]['baseline'] = np.median(tracks[key]['heights'])

        else:
            # Người đang đứng — baseline KHÔNG được cập nhật
            tracks[key]['sit_start'] = None
        ...
        # --- DEBUG: baseline + h_now ---
        cv2.putText(frame, f"base:{baseline}", (x1, y1 - 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
        cv2.putText(frame, f"h:{h}", (x1, y1 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)

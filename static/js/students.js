...
document.addEventListener('DOMContentLoaded', function() {
            ...
            <div class="card mb-3 shadow-sm border-0" style="border-radius: 12px;">
                <div class="card-body d-flex justify-content-between align-items-center p-3">
                    ...
                    <div class="btn-group-sm">
                        ...
                        <button class="btn btn-danger btn-sm px-3" data-action="delete-student" data-id="${s.id}">
    ...
    // ==========================================
    // 2. XỬ LÝ SỰ KIỆN TRÊN DANH SÁCH HỌC SINH
    // ==========================================
    studentList.addEventListener('click', async (e) => {
        ...
        else if (action === 'manage-grades') {
            openGradeModal(id, name);
        } 
        else if (action === 'analyze-student') {
            ...
            if (result) {
                aiAnalysisContent.innerHTML = `
                    <div class="alert alert-info border-0 shadow-sm">
                        <h5 class="fw-bold">Kết luận: ${result.tendency}</h5>
                        <hr>
    ...
    // ==========================================
    // 3. XỬ LÝ THÊM MỚI HỌC SINH (FIXED)
    // ==========================================
    ...
    async function loadGrades(studentId) {
        ...
        gradeListContainer.innerHTML = grades.map(g => `
            <div class="border-bottom py-2 d-flex justify-content-between align-items-center">
                ...
                <div>
                    <button class="btn btn-sm btn-outline-primary" data-action="edit-grade" data-grade-id="${g.id}" data-subject="${g.subject_id}" data-score="${g.score}" data-type="${g.grade_type}" data-term="${g.term}" data-date="${g.exam_date}">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-danger" data-action="delete-grade" data-grade-id="${g.id}">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>`).join('') || '<p class="text-muted mt-2">Chưa có điểm nào.</p>';
    }
    ...
    gradeListContainer.addEventListener('click', async (e) => {
        ...
        } else if (btn.dataset.action === 'edit-grade') {

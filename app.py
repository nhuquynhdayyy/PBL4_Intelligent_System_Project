...
@login_required
def get_current_session_api():
    # CHỈ tìm buổi đang diễn ra của lớp mình
    session = Session.query.filter_by(status='ongoing', class_id=current_user.class_id).first()
    if session:
        counts = db.session.query(Student.student_code, func.count(SpeechLog.id))\
            .join(SpeechLog).filter(SpeechLog.session_id == session.id)\
            .group_by(Student.student_code).all()
        return jsonify({"status": "found", "session_id": session.id, "subject_name": session.subject.name, "speech_counts": dict(counts)})
    return jsonify({"status": "not_found"})
    ...
    try:
        ...
        return jsonify({
            'kpis': {
                ...
                "total_students": Student.query.filter_by(class_id=user_class_id).count(),
                "most_active_student": ranking_query[0][0] if ranking_query and ranking_query[0][1] > 0 else "N/A"
            },
            'all_students_ranking': [{'name': r[0], 'speeches': r[1]} for r in ranking_query],
            'subject_analysis': sub_analysis
        })
    except Exception as e:
        return jsonify({"message": str(e)}), 500

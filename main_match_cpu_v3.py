...
# from recognize_face import recognize_face  # import hàm bạn đã có

# # --- Load model ---
# hand_model = YOLO("runs/detect/train2/weights/best.pt")   # model tay của bạn
# person_model = YOLO("yolov8n.pt")  # model nhận diện người (pretrained)
...
# print("[INFO] Bắt đầu chạy hệ thống (ESC để thoát)...")
...
#         standing = h > 1.15 * baseline  # ngưỡng nhạy hơn
#         if standing:
...
#         print(f"[DEBUG] h={h:.1f}, baseline={baseline:.1f}, stand_frames={tracks[key]['stand_frames']}")

#         # luôn hiển thị zoom khi đứng (dễ thấy trên camera)
#         if standing:
#             fy1, fy2 = y1, y1 + int(0.5 * (y2 - y1))
...
#         # nếu giơ tay và đứng ≥3 frame thì nhận diện
...
person_model = YOLO("yolov8n.pt")  # model detect người (pretrained, nhẹ cho CPU)
...
# --- Bộ nhớ theo dõi đơn giản ---
tracks = {}  # key=(cx,cy) -> {'heights':deque, 'baseline':float, 'stand_frames':int, 'last_recog':float}
...
while True:
    ...
    for pb in person_boxes:
        ...
        h = y2 - y1
        cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
        key = (cx // 20, cy // 20)  # làm tròn tránh rung bbox
        ...
        baseline = tracks[key]['baseline']
        standing = h > 1.15 * baseline  # giảm ngưỡng xuống dễ kích hoạt
        ...
        # --- hiển thị zoom mặt khi đang đứng ---
        if standing:
            fy1, fy2 = y1, y1 + int(0.5 * (y2 - y1))
            face_crop = frame[fy1:fy2, x1:x2]
            if face_crop.size > 0:
